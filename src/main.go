...
func main() {
	...
	go func() {
		...
		var click controllers.ClickController
		var kafka controllers.KafkaController
		mux.HandleFunc("/MqConn", mq.MqConn)
		mux.HandleFunc("/CreateMapping", mq.CreateMapping)
		mux.HandleFunc("/SizeSearch", es.SizeSearch)
		mux.HandleFunc("/SaveData", click.SaveData)
		mux.HandleFunc("/Send", kafka.Send)
		mux.HandleFunc("/Get", kafka.Get)
